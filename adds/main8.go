	...
	if response.StatusCode == 200 {
		...
		for name, values := range response.Header {
			...
			for _, value := range values {
				...
				if name == "Set-Cookie" {
					fmt.Println(name, value)
					getCockieValue(value)
					time.Sleep(920 * time.Millisecond)
					return getMsgsCockie(value)
				}
...
func getMsgsCockie(cockie string) string {
	...
	contiene := "revelio"
	if strings.Contains(bodyMsg, contiene) {
		sendRebelio("rebelio")
	}
...
func sendRebelio(cockie string) string {
	url := "https://makers-challenge.altscore.ai/v1/s1/e8/actions/door"
	client := &http.Client{}
